...
namespace Parrot.client.Mods
{
    internal class Advantage
    {
        ...
        private static GameObject auraRingObj;
        private static LineRenderer auraRingLine;
        private const int AuraSegments = 48;
        public static float auraRingRadius = 0.9f;

        public static void AuraRing()
        {
            if (GorillaTagger.Instance == null || GorillaTagger.Instance.bodyCollider == null)
                return;

            if (auraRingLine == null)
            {
                auraRingObj = new GameObject("AuraRing");
                auraRingObj.hideFlags = HideFlags.HideAndDontSave;

                auraRingLine = auraRingObj.AddComponent<LineRenderer>();
                auraRingLine.useWorldSpace = true;
                auraRingLine.loop = true;
                auraRingLine.positionCount = AuraSegments;
                auraRingLine.widthMultiplier = 0.04f;
                auraRingLine.receiveShadows = false;
                auraRingLine.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;

                Material mat = new Material(Shader.Find("GUI/Text Shader"));
                mat.EnableKeyword("_EMISSION");
                auraRingLine.material = mat;
            }

            Vector3 center = GorillaTagger.Instance.bodyCollider.transform.position + Vector3.down * 0.6f;
            float spin = Time.time * 1.5f;

            Vector3[] pts = new Vector3[AuraSegments];
            for (int i = 0; i < AuraSegments; i++)
            {
                float a = (i / (float)AuraSegments) * Mathf.PI * 2f + spin;
                float wobble = Mathf.Sin(a * 3f + Time.time * 4f) * 0.06f;
                float lift = Mathf.Sin(a * 2f + Time.time * 3f) * 0.05f;
                pts[i] = center + new Vector3(Mathf.Cos(a) * (auraRingRadius + wobble), lift, Mathf.Sin(a) * (auraRingRadius + wobble));
            }
            auraRingLine.SetPositions(pts);

            Color c;
            try { c = Parrot.client.Settings.buttonColors[1].GetCurrentColor(); }
            catch { c = Color.HSVToRGB(Time.time * 0.15f % 1f, 0.8f, 1f); }

            auraRingLine.startColor = c;
            auraRingLine.endColor = c;
            auraRingLine.material.SetColor("_EmissionColor", c);
            ...
            foreach (VRRig rig in VRRigCache.ActiveRigs)
            {
                ...
                if (Vector3.Distance(myPos, rig.transform.position) > auraRingRadius)
                    continue;
        ...
        public static void StopAuraRing()
        {
            if (auraRingObj != null)
                UnityEngine.Object.Destroy(auraRingObj);
            auraRingObj = null;
            auraRingLine = null;
        }
